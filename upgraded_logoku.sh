...
do
 ...
            echo "$satir" >> "$ONEMLI_LOG"  
            # $satir değişkeninde tutulan metni, $ONEMLI_LOG dosyasının sonuna ekliyor.
            break 
            #Satırda ilk anahtar kelimeyi bulur bulmaz, break ile diğer kelimelere bakmayıp döngüyü hemen durduruyoruz.
        fi
    done
done < "$LOG_DOSYASI" 
#while döngüsünün içine giren kod bloğu, standart girdi (standard input) olarak belirtilen dosyadan satır satır veri okur.
